...
func sendEmail(message string, ctx context.Context) {
	// 判断发邮件方式
	emailMethod := getEnvWithDefault("EMAIL_METHOD", "smtp")
...
func sendSMTPEmail(message string) {
	...
	// 从环境变量获取收件人列表，使用逗号分隔
	toEmails := strings.Split(getEnvWithDefault("TO_EMAILS", toEmail), ",")
	mailer.SetHeader("To", toEmails...)
	mailer.SetHeader("Subject", "Webhook Notification")
	mailer.SetBody("text/plain", message)

	dialer := gomail.NewDialer(smtpServer, smtpPort, fromEmail, emailPassword)
